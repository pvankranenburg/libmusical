...
public:
	 ...
	 * Or -1 for a gap
	 */
	virtual int getIx1() const = 0;

	/**
	 * Returns the index _in_ sequence 2.
	 * Or -1 for a gap
	 */
	virtual int getIx2() const = 0;

	/**
	 * Returns index in matrix of the cell the trace is pointing to.
	 */
	virtual int getMatrixPreviousIx1() const = 0;

	/**
	 * Returns index in matrix of the cell the trace is pointing to.
	 */
	virtual int getMatrixPreviousIx2() const = 0;

	/**
	 * Returns index in matrix of the cell the trace element is in
	 */
	virtual int getMatrixThisIx1() const = 0;

	/**
	 * Returns index in matrix of the cell the trace element is in
	 */
	virtual int getMatrixThisIx2() const = 0;

	/**
	 * Returns the accumulatedscore
	 */
	virtual double getAccumulatedscore() const { return accumulatedscore; }

	/**
	 * Returns the score for this particular trace element
	 */
	virtual double getThisscore() const { return thisscore; }
